...
//Station layer
const source_layer = 'stationWithNum_en';
const source_url = "mapbox://" + "shihwenwutw.bmm68zbz"
//Route layer
...
function renderListings(features) {
  ...
  if (radios[0].checked){
    ...
    filteredOnRoutes.forEach(function(feature) {
      ...
      item.insertAdjacentHTML('afterbegin', `<span class="sideList">${stopSeq}</span>`);
...
function featureUpdates_r(value, filtered_input){
  if(value){
    ...
    filtered_input = routes.filter(function(feature) {
      if(exactMatch){
        return value === feature.properties.RouteNameE;
      } else {
        let routes = normalize(feature.properties.RouteNameE);
